...
public abstract class SimpleParticleProvider implements DataProvider {
    private final Map<ResourceLocation, ParticleInfo> particles = new HashMap<>();
    ...
    protected void add(ParticleType<?> particleType, ResourceLocation... textures) {
    ...
    record ParticleInfo(List<ResourceLocation> textures) {
        static final Codec<ParticleInfo> CODEC = RecordCodecBuilder.create(instance -> instance.group(Codec.list(ResourceLocation.CODEC).fieldOf("textures").forGetter(ParticleInfo::textures)).apply(instance, ParticleInfo::new));

        public ParticleInfo(ResourceLocation[] textures) {
