...
public class SimpleBoatItem extends Item {
    private static final Predicate<Entity> ENTITY_PREDICATE = EntitySelector.NO_SPECTATORS.and(Entity::isPickable);
    private final Holder<BoatType> boatType;
    private final boolean hasChest;

    public SimpleBoatItem(boolean hasChest, Holder<BoatType> boatType, Item.Properties properties) {
        super(properties);
    ...
    private Boat getBoat(Level level, HitResult hitResult, ItemStack itemStack, Player player) {
        ...
        Boat boat = this.hasChest ? new SimpleChestBoat(level, vec3.x, vec3.y, vec3.z) : new SimpleBoat(level, vec3.x, vec3.y, vec3.z);
