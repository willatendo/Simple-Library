...
public class SimpleBoatRenderer extends EntityRenderer<Boat> {
    ...
    public SimpleBoatRenderer(EntityRendererProvider.Context context, boolean chestBoat) {
        super(context);
        this.shadowRadius = 0.8F;
        this.modBoatResources = SimpleBuiltInRegistries.BOAT_TYPES.stream().collect(ImmutableMap.toImmutableMap(boatType -> boatType, boatType -> Pair.of(SimpleBoatRenderer.getTextureLocation(boatType, chestBoat), this.createBoatModel(context, boatType, chestBoat))));
    }

    private ListModel<Boat> createBoatModel(EntityRendererProvider.Context context, BoatType boatType, boolean chestBoat) {
        ModelLayerLocation modelLayerLocation = chestBoat ? SimpleModelLayers.createChestBoatModelName(boatType) : SimpleModelLayers.createBoatModelName(boatType);
    ...
    private static ResourceLocation getTextureLocation(BoatType boatType, boolean chestBoat) {
        ResourceLocation boatTypeKey = SimpleBuiltInRegistries.BOAT_TYPES.getKey(boatType);
        return chestBoat ? SimpleUtils.resource(boatTypeKey.getNamespace(), "textures/entity/chest_boat/" + boatType.name() + ".png") : SimpleUtils.resource(boatTypeKey.getNamespace(), "textures/entity/boat/" + boatType.name() + ".png");
    }
