...
public abstract class SimpleTagsProvider<T> implements DataProvider {
    ...
    public SimpleTagsProvider(PackOutput output, String modId, ResourceKey<? extends Registry<T>> registryKey, CompletableFuture<HolderLookup.Provider> lookupProvider) {
        this(output, modId, registryKey, lookupProvider, CompletableFuture.completedFuture(TagsProvider.TagLookup.empty()));
    }

    public SimpleTagsProvider(PackOutput output, String modId, ResourceKey<? extends Registry<T>> registryKey, CompletableFuture<HolderLookup.Provider> lookupProvider, CompletableFuture<TagsProvider.TagLookup<T>> parentProvider) {
    ...
    public CompletableFuture<?> run(final CachedOutput cache) {
        return this.createContentsProvider().thenApply(provider -> {
            ...
            return CompletableFuture.allOf(this.builders.entrySet().stream().map(entry -> {
                ...
                List<TagEntry> entries = tagBuilder.build();
                List<TagEntry> unresolvedEntries = entries.stream().filter(tagEntry -> !tagEntry.verifyIfPresent(elementCheck, tagCheck)).toList();
                if (!unresolvedEntries.isEmpty()) {
                    ...
                    return DataProvider.saveStable(cache, combinedData.contents, TagFile.CODEC, new TagFile(entries, tagBuilder.shouldReplace()), path);
                }
