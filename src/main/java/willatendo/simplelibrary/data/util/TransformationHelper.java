...
public final class TransformationHelper {
	...
	public static class Deserializer implements JsonDeserializer<Transformation> {
		...
		public Transformation deserialize(JsonElement json, Type typeOfT, JsonDeserializationContext context) throws JsonParseException {
			...
			return this.applyOrigin(matrix, new Vector3f(origin));
		}

		private Transformation applyOrigin(Transformation transformation, Vector3f origin) {
			if (transformation.equals(Transformation.identity()))
				return Transformation.identity();

			Matrix4f ret = transformation.getMatrix();
